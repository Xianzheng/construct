...
def table1_view(request):
    ...
    try:
        ...
        return render(request,renderFile,{'modelName':modelName,
                    ...
                    'appName':'基础数据','page_obj':page_obj})
    except:
        renderFile = './table/renderTable1.html'  
        return render(request,renderFile,{'modelName':modelName,
                    'headerAndWidth':'',
                    'totalData':'','status':0,
                    'tableName':'厂区表','tableId':0,
                    'goback':'/logout/','nextLayout':'/'+rootFilePath+'/'+nextModleName,'appName':'基础数据'})
...
def addSubTable_view(request,tableId,tableModel):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tableId == '0':
                return redirect('/基础数据/'+rd+'/')
            return redirect('/基础数据/'+rd+'/'+tableId)
    else:
    ...
    'tableName':title,'goback':goback,'appName':'基础数据'})
...
def updateRow_view(request,modelName,rowId,tableId):
    ...
    'tableId':tableId,'action':action,'goback':goback,'appName':'基础数据'})
