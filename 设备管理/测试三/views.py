...
def table1_view(request):
    ...
    try:
        ...
        #render style
        print(header)
        width = [400 for i in range(len(header))]
        #render到那个template
        ...
        return render(request,renderFile,{'modelName':modelName,
                    ...
                    'totalData':'','status':0,
                    'tableName':'厂区表','tableId':0,
                    'goback':'/logout/','nextLayout':'/'+rootFilePath+'/'+nextModleName,'appName':'测试三'})
...
def addSubTable_view(request,tableId,tableModel):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tableId == '0':
                return redirect('/测试三/'+rd+'/')
            return redirect('/测试三/'+rd+'/'+tableId)
    else:
    ...
    'tableName':title,'goback':goback,'appName':'测试三'})
...
def updateRow_view(request,modelName,rowId,tableId):
    ...
    'tableId':tableId,'action':action,'goback':goback,'appName':'测试三'})
