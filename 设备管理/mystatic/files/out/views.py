...
def table1_view(request):
    ...
    try:
        ...
        return render(request,renderFile,{'modelName':modelName,
                    ...
                    'goback':'/logout/','nextLayout':'/'+rootFilePath+'/'+nextModleName,'appName':'一二三'})
    except:
        renderFile = './table/renderTable1.html'  
        return render(request,renderFile,{'modelName':modelName,
                    'headerAndWidth':'',
...
def addSubTable_view(request,tableId,tableModel):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tableId == '0':
                return redirect('/一二三/'+rd+'/')
            return redirect('/一二三/'+rd+'/'+tableId)
    else:
    ...
    'tableName':title,'goback':goback,'appName':'一二三'})
...
def updateRow_view(request,modelName,rowId,tableId):
    ...
    'tableId':tableId,'action':action,'goback':goback,'appName':'一二三'})
