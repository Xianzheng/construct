...
def table1_view(request):
    ...
    try:
        ...
        return render(request,renderFile,{'modelName':modelName,
                    ...
                    'totalData':'','status':0,
                    'tableName':'厂区表','tableId':0,
                    'goback':'/logout/','nextLayout':'/'+rootFilePath+'/'+nextModleName,'appName':'综合楼'})
...
def addSubTable_view(request,tableId,tableModel):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tableId == '0':
                if 'department=' in url:
                    return redirect('/综合楼/'+rd+'/?department='+department)
                else:
                    return redirect('/综合楼/'+rd+'/')
            
                
            return redirect('/综合楼/'+rd+'/'+tableId)
    else:
    ...
    'tableName':title,'goback':goback,'appName':'综合楼'})
...
def updateRow_view(request,modelName,rowId,tableId):
    ...
    'tableId':tableId,'action':action,'goback':goback,'appName':'综合楼'})
